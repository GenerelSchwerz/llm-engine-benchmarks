...
Do not add `--no-kv-unified`, draft KV types or a forced draft GPU merely because another benchmark used them. Check exact revision help and model-specific load diagnostics; vary each as its own controlled arm. Combined patch stacks are separate engine revisions. Do not attribute results from a stack of Q38/MTP and cache patches to this dependency branch alone; validate this pinned branch independently.
