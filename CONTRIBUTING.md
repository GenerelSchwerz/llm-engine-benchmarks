...
The llama-benchy version is managed in `pyproject.toml` and `uv.lock`, not the engine source list. Update its Git revision deliberately and verify `uv run llama-benchy --help` before changing benchmark claims.
