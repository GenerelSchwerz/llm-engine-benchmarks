...
Each launch has a separate immutable run record. Read `uv run scripts/run_store.py show-run RUN_ID` at the start and whenever context is lost; save its JSON as `run-instructions.json` inside the recorded output directory. The record contains the run-specific description and optional VRAM ceiling in GiB. Enforce the ceiling against sampled **total-device** peak GPU memory, and record process memory too where possible. After each arm, call `uv run scripts/run_store.py record-peak RUN_ID ENGINE_ID ARM --peak-total-mib N`; the CLI stores the observation and exits nonzero when it exceeds the ceiling. Treat that arm as over budget. Different ceilings are separate runs even when the suite, model, and prompts match. The run description may request tuning, but never silently rewrite frozen packets: retain exact candidate commands, reasons for each change, and measured outcomes under the new run ID. Compare only arms with compatible memory budgets, workload, and hardware.
...
4. Verify startup, health, memory fit, the output sanity gate below, and any claimed optimization path before timing. An allocation or offload count alone is not proof that a cache or draft path executed.
...
Use frozen, nonprivate tasks from `fixtures/` or add suite-specific fixtures with hashes. Include at least a coding task and a technical explanation; use a long-context task when supported. Preserve exact request/invocation, streaming chunks when available, visible answer **and reasoning** fields, token accounting, stop reason, prompt hash, and a short quality verdict with concrete defects.

**Coherence here is a minimal output sanity gate:** inspect generated text from `content`, `reasoning_content`, `reasoning`, or the engine's equivalent channel. Pass if generation is substantive and relevant enough to rule out an immediately obvious failure such as empty output across all channels, garbage, a repetition loop, or an API error. Reasoning-only text is valid generated output. A fixed token limit may end with `finish_reason=length`; that alone does not fail this gate. Record `answer_complete` and `task_correct` separately. Missing visible answer text, a cut-off answer, or a wrong solution must be reported, but none automatically blocks throughput measurement after the sanity gate passes. Do not call those outputs successful task completions or use them for quality-matched speed claims.

Match prompt, token ceiling, sampling, concurrency, context geometry, and hardware/memory cohort across comparisons. Engines may need different launch flags. Report TTFT, prefill, client-perceived decode, wall time, RAM, VRAM, and errors. Count and time reasoning tokens as generated tokens; retain channel breakdown where available. Record cache/draft counters only for claims about those features. If engine outputs differ materially or task answers are incomplete, treat speed differences as observational until quality is evaluated.

## Method B: pinned llama-benchy

Use the `llama-benchy` package pinned in `pyproject.toml` and `uv.lock`, as described in [tooling.md](manifests/tooling.md). Install it with `uv sync`. It sends OpenAI-compatible `/v1/chat/completions` requests. Verify the engine's endpoint and exact served model alias, or pin a protocol adapter and document its semantics and overhead. Its built-in probe searches the response's answer **and reasoning** for a specific fact; leave it enabled when it passes. If it fails despite a documented independent sanity pass, record the raw probe failure and use `--skip-coherence` for that arm, with the reason in the result. Never treat the tool's probe as a substitute for Method A.
