...
Git clones are installed from `manifests/sources.json` with `uv run --locked scripts/install_sources.py`; the manifest also supports non-Git entries with explicit installation notes. Never update a source checkout in the middle of a suite.
...
Use the `llama-benchy` package pinned in `pyproject.toml` and `uv.lock`, as described in [tooling.md](manifests/tooling.md). Install it with `uv sync --locked`. It sends OpenAI-compatible `/v1/chat/completions` requests. Verify the engine's endpoint and exact served model alias, or pin a protocol adapter and document its semantics and overhead. Keep the tool's coherence probe enabled, but still run Method A independently.
