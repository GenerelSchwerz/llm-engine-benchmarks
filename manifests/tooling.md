...
The editable install pin is in [`sources.json`](sources.json), with kind `tool`; run `uv run scripts/install_sources.py --id llama-benchy` to create the ignored `tools/llama-benchy` checkout. Its [README](https://github.com/eugr/llama-benchy) documents `--pp`, `--tg`, `--depth`, `--runs`, `--concurrency`, `--tokenizer`, JSON output, and a built-in coherence probe. It only uses `/v1/chat/completions`. The built-in probe does not replace the separate coherent-text track in this suite.
