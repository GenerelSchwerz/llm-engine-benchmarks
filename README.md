...
In **Agents**, select a pane and press **Close selected agent** to remove it. Closing a running pane stops that agent and its private tmux session; closing an exited pane clears it from the panel. Saved setup records, run artifacts, and findings remain available.
